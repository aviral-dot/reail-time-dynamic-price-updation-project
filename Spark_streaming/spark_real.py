...
def main():
    try:
        ...
        ).withColumn("_id", col("region"))\
        .select("timestamp", "region", "competitor_price", "predicted_price", "_id")

        print("Starting streaming query")
        def upsert_to_mongo(batch_df, epoch_id):
         print(f"Writing batch {epoch_id} to MongoDB with {batch_df.count()} rows")
         batch_df.write \
        .format("mongodb") \
        .mode("append") \
        .option("spark.mongodb.connection.uri", "mongodb://admin:password@mongo1:27017,mongo2:27017/pricing.price_history?replicaSet=rs0&authSource=admin") \
        ...
        .option("checkpointLocation", "file:///tmp/checkpoints/mongo_dynamic_pricing") \
